...
export async function GET(request:NextRequest){
  ...
  try{
    ...
    if(!res.ok){
      ...
      console.error('[SORARE] HTTP '+res.status,body.slice(0,200));
      return NextResponse.json({error:'Sorare HTTP '+res.status},{status:500});
    }
    const json=await res.json();
    if(json.errors||!json?.data?.user?.cards){
      const msg=json.errors?JSON.stringify(json.errors).slice(0,300):'user null';
      console.error('[SORARE ERR]',msg);
    ...
    console.error('[SORARE CATCH]',String(e));
