...
export async function GET(request:NextRequest){
  ...
  try{
    const r=await fetch('https://api.sorare.com/federation/graphql',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({query}),signal:AbortSignal.timeout(8000)});
    if(!r.ok)return NextResponse.json({error:'sorare_error'},{status:502});
    const j=await r.json();
    if(j.errors?.length){console.error('[cards route]',j.errors[0].message);return NextResponse.json({error:j.errors[0].message},{status:400});}
