...
const StatPanel=memo(({card,onClose,isClosing=false}:any)=>{
  ...
  const fv=(v:any,t:string)=>{if(v==null)return '\u2014';const nn=Number(v);if(t==='pct')return nn.toFixed(1)+'%';if(t==='pm')return(nn>=0?'+':'')+nn.toFixed(1);if(t==='int')return String(Math.round(nn));return nn.toFixed(1);};
  return(
