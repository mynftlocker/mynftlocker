...
export default function Home() {
  ...
  return(
    <main style={{minHeight:'100vh',background:contentBg,backgroundAttachment:'fixed',color:'white',fontFamily:'system-ui,sans-serif',display:'flex'}}>
      ...
      <aside style={{position:'fixed',left:0,top:0,width:'220px',height:'100vh',overflow:'visible',background:sideBg,backdropFilter:'blur(12px)',WebkitBackdropFilter:'blur(12px)',borderRight:'1px solid rgba(111,195,232,0.35)',boxShadow:'1px 0 0 rgba(111,195,232,0.25), 6px 0 24px -6px rgba(111,195,232,0.22)',padding:'0.4rem 0.6rem',zIndex:50,scrollbarWidth:'none',display:'flex',flexDirection:'column' as const,gap:'0',fontFamily:"'Inter','Segoe UI',system-ui,-apple-system,sans-serif"}}>
        ...
        {cards.length>0&&(
          <div style={{textAlign:'left',flex:1,display:'flex',flexDirection:'column' as const,gap:0}}>
            ...
            <p style={{fontSize:'0.6rem',fontWeight:800,letterSpacing:'0.14em',textTransform:'uppercase',color:'rgba(255,255,255,0.45)',margin:'0 0 0.22rem',padding:'0 0.5rem'}}>SPORT</p>
            <div style={{display:'flex',gap:'0.35rem',flexWrap:'wrap',marginBottom:'0.5rem',padding:'0 0.5rem'}}>
              {(['nba','foot'] as const).map(s=>(
                <button key={s} onClick={()=>{setGSport(s);setGLeague('all');setGTeamCustom('all');setGLeagueOpen(false);setGClubOpen(false);}}
                  style={{padding:'0.2rem 0.55rem',border:'1px solid',borderRadius:'2rem',cursor:'pointer',fontSize:'0.68rem',fontWeight:700,letterSpacing:'0.08em',transition:'all 0.18s',background:'transparent',borderColor:gSport===s?'rgba(0,225,255,0.8)':'rgba(255,255,255,0.15)',color:gSport===s?'#00e5ff':'rgba(255,255,255,0.5)',boxShadow:gSport===s?'0 0 8px rgba(0,225,255,0.25)':'none'}}
                >{s.toUpperCase()}</button>
              ))}
            </div>
            <p style={{fontSize:'0.6rem',fontWeight:800,letterSpacing:'0.14em',textTransform:'uppercase',color:'rgba(255,255,255,0.45)',margin:'0 0 0.22rem',padding:'0 0.5rem'}}>ÉQUIPE</p>
            <div style={{position:'relative',marginBottom:'0.65rem',padding:'0 0.5rem'}}>
              <div onClick={()=>setGClubOpen((o:boolean)=>!o)} style={{display:'flex',alignItems:'center',justifyContent:'space-between',background:'rgba(10,10,12,0.95)',border:'1px solid',borderColor:gClubOpen?'rgba(0,225,255,0.7)':'rgba(0,225,255,0.3)',borderRadius:'0.2rem',padding:'0.28rem 0.5rem',cursor:'pointer',fontSize:'0.68rem',color:'rgba(255,255,255,0.8)',transition:'border-color 0.15s'}}>
                <span style={{overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',flex:1}}>{(galleryTeamList as any[]).find((t:any)=>t.api===gTeamCustom)?.display||'Toutes les équipes'}</span>
              ...
              {gClubOpen&&(
                <div className='gal-scroll' style={{position:'absolute',top:0,left:'calc(100% + 4px)',width:'190px',background:'rgba(8,8,12,0.98)',border:'1px solid rgba(0,225,255,0.35)',borderRadius:'0.2rem',zIndex:400,maxHeight:'70vh',overflowY:'auto'}}>
                  <div onClick={()=>{setGTeamCustom('all');setGClubOpen(false);}} style={{padding:'0.36rem 0.55rem',fontSize:'0.7rem',cursor:'pointer',background:gTeamCustom==='all'?'rgba(0,225,255,0.1)':'transparent',color:gTeamCustom==='all'?'#00e5ff':'rgba(255,255,255,0.75)',transition:'background 0.12s'}} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background='rgba(0,225,255,0.07)';}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background=gTeamCustom==='all'?'rgba(0,225,255,0.1)':'transparent';}}>Toutes les équipes</div>
                  {(galleryTeamList as any[]).map((t:any)=>(
                    <div key={t.api} onClick={()=>{setGTeamCustom(t.api);setGClubOpen(false);if(mode==='locker'){setTeamApi(t.api);}}} style={{padding:'0.36rem 0.55rem',fontSize:'0.7rem',cursor:'pointer',background:t.api===gTeamCustom?'rgba(0,225,255,0.1)':'transparent',color:t.api===gTeamCustom?'#00e5ff':'rgba(255,255,255,0.75)',transition:'background 0.12s'}} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background='rgba(0,225,255,0.07)';}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background=t.api===gTeamCustom?'rgba(0,225,255,0.1)':'transparent';}}>{t.display}</div>
                  ))}
                </div>
              )}
            </div>
            {mode==='gallery'&&(
              ...
              <p style={{fontSize:'0.6rem',fontWeight:800,letterSpacing:'0.14em',textTransform:'uppercase',color:'rgba(255,255,255,0.45)',margin:'0 0 0.22rem',padding:'0 0.5rem'}}>TRI</p>
              <div style={{display:'flex',flexDirection:'column' as const,overflow:'hidden',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'0.2rem'}}>
                {([['default','Défaut'],['rarity','Rareté'],['score','Score L10'],['name','Nom']] as [string,string][]).map(([v,l])=>(
                  <button key={v} onClick={()=>setGSort(v as any)} style={{padding:'0.22rem 0.5rem',border:'none',borderBottom:'1px solid rgba(255,255,255,0.06)',cursor:'pointer',fontSize:'0.64rem',textAlign:'left' as const,background:gSort===v?'rgba(212,175,55,0.18)':'transparent',color:gSort===v?'#e8c84a':'rgba(255,255,255,0.5)',fontWeight:gSort===v?700:400,transition:'all 0.15s'}}>{l}</button>
                ))}
              </div>
              </div>
            )}
          ...
          <div style={{position:'fixed',inset:0,zIndex:100,display:'flex',flexDirection:'column' as const,alignItems:'center',justifyContent:'center',overflow:'hidden'}}>
            ...
            <div style={{position:'relative',zIndex:1,marginBottom:openPhase===0?'2.8rem':'1.5rem'}}
              ...
              <img src='/logo-pack.png' alt='myNFTlocker'
                style={{
                  width:'280px',display:'block',position:'relative',zIndex:1,background:'transparent',
              ...
              {openPhase===1&&(
                <svg style={{position:'absolute',inset:0,width:'100%',height:'100%',overflow:'visible',pointerEvents:'none',zIndex:3}}>
                  <rect x='6%' y='1%' width='88%' height='98%' rx='10'
                    fill='none'
                    stroke='#00ffff'
                    strokeWidth='2'
                    pathLength='1000'
                    style={{
                      ...
                      strokeDashoffset:0,
                      filter:'drop-shadow(0 0 5px rgba(0,225,255,0.95)) drop-shadow(0 0 10px rgba(0,225,255,0.55))',
                      animation:'scannerLaser 2.2s linear infinite',
                    }}/>
                </svg>
              )}
            </div>
