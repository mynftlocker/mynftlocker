...
const COUNTRY_FR: Record<string,string> = {
  ...
  'brazil':'Brésil','bulgaria':'Bulgarie','cameroon':'Cameroun',
  'chile':'Chili','china':'Chine','colombia':'Colombie',
  'croatia':'Croatie','czech-republic':'République Tchèque',
  'denmark':'Danemark','ecuador':'Équateur','egypt':'Égypte',
  ...
  'united-states':'États-Unis','uruguay':'Uruguay','venezuela':'Venezuela',
...
const FootTeamMenu=({countries,teamsByCountry,country,team,onPickCountry,onPickTeam}:any)=>{
  ...
  const label=team!=='all'?team:(country!=='all'?countryFR(country):'Toutes les équipes');
  ...
  const menuStyle=(leftOffset:number):React.CSSProperties=>({
    position:'fixed',left:leftOffset+'px',top:posT+'px',
    maxHeight:Math.min(360,window.innerHeight-posT-16)+'px',
  ...
  return(
    <div ref={ref} style={{position:'relative',marginBottom:'0.15rem'}}
      ...
      {open&&mounted&&createPortal(
        <div style={{position:'fixed',left:posL+'px',top:'0',bottom:'0',zIndex:9998,display:'flex',pointerEvents:'none'}}>
          {/* Menu Pays */}
          <div className='thin-sb' style={{...menuStyle(posL),position:'relative',left:'auto',top:'auto',marginTop:posT+'px',pointerEvents:'all',alignSelf:'flex-start'}}>
            <button style={btnStyle(country==='all'&&team==='all')} onClick={()=>{onPickCountry('all');onPickTeam('all');setHoveredCountry(null);}}>Toutes les équipes</button>
            {sortedCountries.map((c:string)=>(
              <button key={c} style={{...btnStyle(country===c),display:'flex',justifyContent:'space-between',alignItems:'center'}}
          ...
          {/* Menu Équipes (affiché si un pays est survolé) */}
          {hoveredCountry&&(teamsByCountry[hoveredCountry]||[]).length>0&&(
            <div className='thin-sb' style={{...menuStyle(posL+200),position:'relative',left:'auto',top:'auto',marginTop:posT+'px',pointerEvents:'all',alignSelf:'flex-start',marginLeft:'4px'}}>
